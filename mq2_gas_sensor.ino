...
void loop()
{
  Blynk.run();
   ...
   Blynk.virtualWrite(V1, MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_LPG));
   Blynk.virtualWrite(V2, MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_CO));
   Blynk.virtualWrite(V3, MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_SMOKE));
   ...
   if(s1 == HIGH){  // FORWARD
    ...
    if(s2 == HIGH){  // REVERSE
     ...
     if(MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_CO)>=80){ //set temperature level for notification
     ...
     if(MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_CO)>=120){
     digitalWrite(4, HIGH);
     digitalWrite(5, HIGH);
     digitalWrite(6, HIGH);
     digitalWrite(7, HIGH);
